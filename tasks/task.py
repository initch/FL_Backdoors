...
class Task:
    ...
    def make_poison_optimizer(self, model=None) -> Optimizer:
        if model is None:
            model = self.model
        if self.params.optimizer == 'SGD':
            optimizer = optim.SGD(model.parameters(),
                                  lr=self.params.poison_lr,
                                  weight_decay=self.params.decay,
                                  momentum=self.params.momentum)
        elif self.params.optimizer == 'Adam':
            optimizer = optim.Adam(model.parameters(),
    ...
    def make_scheduler(self) -> None:
        if self.params.scheduler == 'CosineAnnealingLR':
            ...
            self.scheduler = MultiStepLR(self.optimizer, self.params.scheduler_milestones)
        elif self.params.scheduler == 'ReduceLROnPlateau':
            self.scheduler = ReduceLROnPlateau(self.optimizer, patience=3)
        else:
            raise ValueError(f'Unsupported scheduler: {self.params.scheduler}')
