...
class MNISTFedTask(FederatedLearningTask, MNISTTask):
    ...
    def sample_dirichlet_train_data(self, no_participants, alpha=0.9):
        """
            ...
            Requires: mnist_classes, a preprocessed class-indices dictionary.
        ...
        no_classes = len(mnist_classes.keys())

        for n in range(no_classes):
            random.shuffle(mnist_classes[n])
            sampled_probabilities = class_size * np.random.dirichlet(
                np.array(no_participants * [alpha]))
            for user in range(no_participants):
                no_imgs = int(round(sampled_probabilities[user]))
                sampled_list = mnist_classes[n][
                               :min(len(mnist_classes[n]), no_imgs)]
                per_participant_list[user].extend(sampled_list)
                mnist_classes[n] = mnist_classes[n][
                                   min(len(mnist_classes[n]), no_imgs):]
